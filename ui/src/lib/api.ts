const resolveBaseUrl = (): string => {
  ...
  if (typeof window !== "undefined") {
    ...
    // Local dev server: default backend runs on 8000.
    if (import.meta.env.DEV && port !== "8000") {
      return `${protocol}//${hostname}:8000`;
    }
    // Production/static hosting: call same-origin backend.
  ...
  async startSession(topic: string, options: StartSessionOptions = {}) {
    return this.request("/api/sessions", {
      ...
      body: {
        ...
        strict_grounding: options.strictGrounding ?? true,
        enforce_numeric_citations: options.enforceNumericCitations ?? true,
        judge_replay: options.judgeReplay ?? false,
        replay_session_id: options.replaySessionId,
      },
    });
  }

  async listReplaySessions(limit: number = 20) {
    return this.request(`/api/replay/sessions?limit=${limit}`);
  }
