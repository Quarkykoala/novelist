...
const CONFIG = {
    ...
    PHEROMONE_DEPOSIT: 5,
    PHEROMONE_DECAY: 0.9,
    TRAIL_COLOR: { r: 64, g: 224, b: 208 }, // Turquoise
...
export function PhysarumCanvas({
    isActive,
    ...
    const initializeAgents = useCallback(() => {
        ...
        // Use provided center or fallback to screen center
        let startX = centerX;
        let startY = centerY;
        ...
        for (let i = 0; i < CONFIG.AGENT_COUNT; i++) {
            ...
            agents.push({
                x: startX + Math.cos(angle) * offsetRadius,
                y: startY + Math.sin(angle) * offsetRadius,
                angle: angle,
    ...
    const senseChemical = useCallback((x: number, y: number, currentCenterX: number, currentCenterY: number): number => {
        ...
        if (map && x >= 0 && x < dimensionsRef.current.width && y >= 0 && y < dimensionsRef.current.height) {
    ...
    const simulationStep = useCallback(() => {
        ...
        // This avoids creating new RadialGradient objects and loop iterations every frame.
