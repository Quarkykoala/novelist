...
export function Dashboard() {
  ...
  const [enforceNumericCitations, setEnforceNumericCitations] = useState(true);
  const [judgeReplay, setJudgeReplay] = useState(false);
  ...
  const [sessionFlags, setSessionFlags] = useState<{
    ...
    enforceNumericCitations?: boolean;
    judgeReplay?: boolean;
  ...
  const loadReplaySessions = async () => {
    try {
      const data = await api.listReplaySessions(20);
      const sessions = data?.sessions || [];
      setReplaySessions(sessions);
      if (!replaySessionId && sessions.length > 0) {
        setReplaySessionId(sessions[0].session_id);
      }
    } catch (err: any) {
      addLog("error", "Failed to load replay sessions: " + err.message);
    }
  };
  ...
  const handleGenerate = async () => {
    ...
    if (judgeReplay && replaySessions.length === 0) {
    ...
    if (sessionId && isGenerating) {
      pollInterval.current = setInterval(async () => {
        try {
          ...
          setSessionFlags({
            strictGrounding: s.strict_grounding,
            enforceNumericCitations: s.enforce_numeric_citations,
            judgeReplay: s.judge_replay,
            replaySourceSessionId: s.replay_source_session_id || null,
          });
          ...
          if (s.knowledge_sources) setKnowledgeSources(s.knowledge_sources);
          if (s.source_breakdown) setSourceBreakdown(s.source_breakdown);
          ...
          const isErrorState = s.status === "error" || phaseValue === "error" || Boolean(s.error);
          const errorText = s.error || s.status_detail || "Session failed.";
          ...
          <CardContent className="space-y-4">
            ...
            <div className="space-y-2">
              ...
              <label className="flex items-center gap-2 text-xs">
                ...
                />
                Strict grounding (drop unsupported hypotheses)
              </label>
              <label className="flex items-center gap-2 text-xs">
                <input
                  type="checkbox"
                  checked={enforceNumericCitations}
                  onChange={(e) => setEnforceNumericCitations(e.target.checked)}
                />
                Enforce numeric citation coverage
              </label>
              <label className="flex items-center gap-2 text-xs">
                <input
                  type="checkbox"
                  checked={judgeReplay}
                  onChange={(e) => setJudgeReplay(e.target.checked)}
                />
                Judge replay mode (deterministic demo)
              </label>
              {judgeReplay && (
                <div className="space-y-2 rounded-md border border-border p-2">
                  <div className="text-[10px] uppercase tracking-wider text-muted-foreground">
                    Replay Source Session
                  </div>
                  <div className="flex gap-2">
                    <select
                    ...
                    >
                      {replaySessions.length === 0 && (
                        <option value="">No replay sessions found</option>
                      )}
                    ...
                    <Button variant="outline" size="sm" onClick={loadReplaySessions}>
                      Refresh
                    </Button>
                  </div>
                </div>
                ...
                <div className="flex flex-wrap gap-2">
                  {knowledgeSources.length > 0 ? knowledgeSources.map((source) => (
                    <Badge key={source} variant="secondary" className="capitalize">
                      {source}
                      {sourceBreakdown[source] ? ` (${sourceBreakdown[source]})` : ""}
                    </Badge>
                  )) : (
                    <Badge variant="outline">Awaiting retrieval</Badge>
                  )}
                </div>
            </div>
            ...
            {(sessionFlags.judgeReplay || (phase === "complete" && hypotheses.length === 0 && sessionFlags.strictGrounding)) && (
              <div className="rounded-md border bg-amber-50/80 p-3 space-y-1 text-xs">
                {sessionFlags.judgeReplay && (
                  <p className="text-amber-800">
                    Replay mode active{sessionFlags.replaySourceSessionId ? ` (source: ${sessionFlags.replaySourceSessionId})` : ""}.
                  </p>
                )}
                {phase === "complete" && hypotheses.length === 0 && sessionFlags.strictGrounding && (
                  <p className="text-amber-800">
                    Zero surviving hypotheses. Strict grounding removed candidates without valid pulled-paper citations.
                  </p>
                )}
              </div>
            )}
