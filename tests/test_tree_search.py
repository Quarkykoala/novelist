...
class TestTreeSearch(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_tree_search_flow(self):
        ...
        generator.generate_batch = AsyncMock(side_effect=[
            [GroundedHypothesis(
                id="h1", claim="Hypothesis 1 with sufficient length for validation.",
                mechanism=[
                ...
                null_result="No significant increase is observed in outcome D.",
                source_soul=SoulRole.CREATIVE,
                gap_addressed="Gap 1 in the research literature linking A and B.",
                supporting_papers=[],
        ...
        root_state = ResearchState(
            gaps=[
                IdentifiedGap(
                    ...
                    description="Gap 1 describes a missing connection between A and B with sufficient detail.",
                    concept_a="A",
                    concept_b="B",
                    potential_value="High impact connection to explore.",
                ),
            ],
            depth=0
        )
