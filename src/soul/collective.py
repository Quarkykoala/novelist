...
class SoulCollective:
    ...
    async def run_debate(
        ...
        creative_samples = [h for h in proposals if h.source_soul == SoulRole.CREATIVE][:2]
        for sample in creative_samples:
            trace["dialogue"].append(DialogueEntry(
                soul=creative_name,
                role=SoulRole.CREATIVE,
                message=f"Proposal {sample.id}: {_short(sample.hypothesis)}"
            ))

        risk_samples = [h for h in proposals if h.source_soul == SoulRole.RISK_TAKER][:2]
        for sample in risk_samples:
            trace["dialogue"].append(DialogueEntry(
                soul=maverick_name,
                role=SoulRole.RISK_TAKER,
                message=f"High-upside proposal {sample.id}: {_short(sample.hypothesis)}"
            ))
        ...
        critiques, survivors = await self._phase_critique(proposals, topic, context)
        ...
        for critique in critiques[:3]:
            if critique.verdict == CritiqueVerdict.PASS:
                continue
            issue_preview = "; ".join(critique.issues[:2]) if critique.issues else "No explicit issue text."
            trace["dialogue"].append(DialogueEntry(
                soul="The Skeptic",
                role=SoulRole.SKEPTIC,
                message=(
                    f"Critique {critique.hypothesis_id} -> {critique.verdict.value.upper()}: "
                    f"{_short(issue_preview, 220)}"
                ),
            ))
        ...
        trace["dialogue"].append(DialogueEntry(
            ...
            message=(
                f"Protocol review complete: {len(enhanced)}/{len(survivors)} hypotheses "
                "have structured experimental designs."
            ),
        ))
        ...
        for sample in final[:3]:
            refs = ", ".join((sample.supporting_papers or [])[:2]) or "no citations attached"
            trace["dialogue"].append(DialogueEntry(
                soul="The Synthesizer",
                role=SoulRole.SYNTHESIZER,
                message=f"Final {sample.id}: {_short(sample.hypothesis, 160)} (refs: {refs})",
            ))
    ...
    async def _phase_critique(
        ...
        critiques = await self.skeptic.critique(proposals, topic, context=context)
