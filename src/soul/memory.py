...
class Graveyard:
    ...
    Enhanced with semantic similarity search and anti-pattern extraction.
    """
    ...
    def exhume(self, topic: str, limit: int = 5) -> list[str]:
        """Get relevant failed ideas using Jaccard similarity.
        
        Enhanced: Uses word overlap scoring instead of simple keyword matching.
        """
        if not self.ghosts:
            return []
        ...
        for ghost in self.ghosts:
            # Combine hypothesis and topic words for matching
            ghost_hyp_words = set(ghost.get("hypothesis", "").lower().split())
            ghost_topic_words = set(ghost.get("topic", "").lower().split())
            all_ghost_words = ghost_hyp_words | ghost_topic_words
            
            # Jaccard similarity: intersection / union
            intersection = len(topic_words & all_ghost_words)
            union = len(topic_words | all_ghost_words)
            score = intersection / union if union > 0 else 0.0
            
            if score > 0.05:  # Minimum threshold to be considered relevant
                scored.append((score, ghost))
        ...
        return [
            f"FAILED IDEA: {g['hypothesis'][:100]}... (REASON: {g['reason']})"
            for _, g in scored[:limit]
        ]

    def get_anti_patterns(self, limit: int = 3) -> list[str]:
        """Extract common failure reasons as anti-patterns for prompts.
        
        Clusters failures by reason and returns the most common ones
        to help future generations avoid similar mistakes.
        """
        ...
        reasons = [g.get("reason", "").strip() for g in self.ghosts if g.get("reason")]
        if not reasons:
            return []
        
        # Count occurrences
        reason_counts = Counter(reasons)
        common = reason_counts.most_common(limit)
        
        return [f"Avoid: {reason}" for reason, count in common if count >= 2]

    def get_failure_stats(self) -> dict[str, int]:
        """Get statistics about failure patterns."""
        from collections import Counter
        
        reasons = [g.get("reason", "unknown") for g in self.ghosts]
...
class MemorySystem:
    ...
    def get_context_for_generation(self, topic: str = "") -> dict[str, Any]:
        ...
        Compiles information from all memory types, including anti-patterns
        from past failures.
        """
        return {
            ...
            "anti_patterns": self.graveyard.get_anti_patterns(),  # NEW: Common failure patterns
