...
def format_context_for_prompt(context: dict[str, Any]) -> str:
    ...
    if context.get("source_papers"):
        parts.append("PULLED PAPERS (CITE ONLY THESE IDS):")
        for paper in context["source_papers"][:12]:
            paper_id = paper.get("id", "")
            title = paper.get("title", "")
            source = paper.get("source", "")
            if paper_id and title:
                source_label = f" ({source})" if source else ""
                parts.append(f"  - [{paper_id}] {title}{source_label}")

    if context.get("source_claims"):
        parts.append("GROUNDING CLAIMS FROM PULLED PAPERS:")
        for claim in context["source_claims"][:10]:
            paper_id = claim.get("paper_id", "")
            statement = claim.get("statement", "")
            if paper_id and statement:
                parts.append(f"  - [{paper_id}] {statement}")

    if context.get("citation_policy"):
        parts.append(f"CITATION POLICY: {context['citation_policy']}")
...
[
  {
    ...
    "evidence_trace": ["[paper_id_1] short supporting claim"],
    "supported_facts": ["[paper_id_1] direct fact from source", "[paper_id_2] direct fact from source"],
    "novel_inference": "How the supported facts are combined into a new hypothesis",
    "unsupported_parts": ["Any uncertain claim fragments that still need evidence"],
    "evidence_spans": [
      {
        "claim_text": "Exact claim sentence or fragment supported",
        "citation_id": "paper_id_1",
        "quote": "Short quote/span from title or abstract supporting the claim",
        "confidence": 0.8
      }
    ]
  }
...
- supporting_papers must include 1-3 IDs from the pulled paper list in context
- evidence_trace must include at least one line with a cited pulled-paper ID
- supported_facts must be direct source-backed statements
- novel_inference must explicitly describe the new synthesis step
- unsupported_parts must list any still-unproven sub-claims (empty list if none)
- evidence_spans must map claim_text -> citation_id -> quote for each claim sentence
