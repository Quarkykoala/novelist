...
"""
    ...
    async def critique(
        ...
        """
        # Phase 1: Pulled-corpus verification (fail-closed, no external fetches)
        verification_context = self._verify_claims_from_context(hypotheses, context or {})
    ...
    def _tokenize(text: str) -> set[str]:
        ...
        stop = {
            ...
            "have", "has", "had", "can", "could", "would", "should", "are", "was",
            "were", "battery", "batteries", "energy", "effect", "effects",
        }
        tokens = re.findall(r"[a-z0-9][a-z0-9\-]{2,}", text.lower())
        return {token for token in tokens if token not in stop}

    def _verify_claims_from_context(
        self,
        hypotheses: list[Hypothesis],
        context: dict[str, Any],
    ) -> dict[str, list[dict[str, str]]]:
        """Rank pulled papers against each hypothesis without external retrieval."""
        source_papers = context.get("source_papers") or []
        if not source_papers:
            return {h.id: [] for h in hypotheses}

        results: dict[str, list[dict[str, str]]] = {}
        for h in hypotheses:
            hyp_tokens = self._tokenize(
                " ".join([h.hypothesis] + (h.novelty_keywords or []))
            )
            scored: list[tuple[float, dict[str, str]]] = []
            for paper in source_papers:
                title = paper.get("title", "")
                abstract = paper.get("abstract", "")
                corpus_tokens = self._tokenize(f"{title} {abstract}")
                overlap = hyp_tokens & corpus_tokens
                if not overlap:
                    continue
                title_hits = sum(1 for token in hyp_tokens if token in title.lower())
                score = float(len(overlap) + 0.5 * title_hits)
                scored.append(
                    (
                        score,
                        {
                            "id": paper.get("id", ""),
                            "title": title,
                            "summary": (abstract or "")[:200],
                        },
                    )
                )
            scored.sort(key=lambda item: item[0], reverse=True)
            results[h.id] = [paper for _, paper in scored[:3]]
        return results
...
FOUND LITERATURE (Pulled Corpus):
...
USE THE "FOUND LITERATURE" (pulled corpus only) to fact-check the claims.
